...
- **假人的召唤**
  - **指令格式**：`/player <假人id> spawn [at <X坐标> <Y坐标> <Z坐标> [facing <偏航角> <仰俯角> [in <维度ID>]]]`
  - **指令解读**：<必填> [选填]
...
- **[点此查看MC百科原文/详细使用](https://www.mcmod.cn/class/2361.html)**  
...
### 正确使用顺序
 ...
 - **5.以输入`/player bot_huanse use`后开始计时，需等待35秒后才可开机**
 - **6.等待35秒后，输入以下指令踢出换色用假人**  
 ...
 - **9.以输入`/player bot_guhuamoshi use`开始计时，需等待10秒后才可开机**
 - **10.等待10秒后，输入以下指令踢出固化模式切换用假人**  
